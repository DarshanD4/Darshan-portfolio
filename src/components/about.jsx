...
function About() {
  const highlights = [
    ...
    },
    {
      ...
      icon: (
        <svg className="w-5 h-5 text-rose-600" fill="none" stroke="currentColor" strokeWidth="2.2" viewBox="0 0 24 24">
  ...
  return (
    <section id="about" className="py-24 px-4 sm:px-8 bg-slate-50/70 border-t border-slate-200/70">
      <div className="max-w-6xl mx-auto">
        ...
        <div className="grid gap-12 lg:grid-cols-12 items-start">
          {/* Story Narrative */}
          <div className="lg:col-span-6 space-y-5 text-slate-600 leading-relaxed text-base">
            <p>
              I am an App Developer Intern at{" "}
              <a
                href="https://bytecraftstudios.in/"
                target="_blank"
                rel="noreferrer"
                className="font-bold text-slate-900 underline decoration-sky-500 decoration-2 hover:text-sky-600 transition"
              >
                Bytecraft Studios
              </a>
            ...
            <div className="pt-3">
              <a
                ...
                target="_blank"
                rel="noreferrer"
                className="inline-flex items-center gap-2 text-sm font-bold text-slate-900 hover:text-sky-600 transition"
              >
          ...
          <div className="lg:col-span-6 grid gap-4 sm:grid-cols-2">
            {highlights.map((item) => (
              <div
                key={item.title}
                className="pro-card p-6 bg-white flex flex-col justify-between"
              >
                <div>
                  <div className="w-10 h-10 rounded-xl bg-slate-50 border border-slate-200/80 flex items-center justify-center mb-4 shadow-2xs">
                    {item.icon}
                  </div>
