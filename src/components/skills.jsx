...
function Skills() {
  ...
  const getIcon = (name) => {
    const iconClass = "w-6 h-6 text-slate-800 transition-transform group-hover:scale-110";
    switch (name) {
      case "Flutter":
        return (
  ...
  return (
    <section id="skills" className="py-24 px-4 sm:px-8 bg-white border-t border-slate-200/70">
          ...
          <AnimatePresence mode="popLayout">
            {filteredSkills.map((skill) => (
              <motion.div
                ...
                className="pro-card p-5 bg-slate-50/50 hover:bg-white flex flex-col items-center justify-center text-center group cursor-default"
              >
                <div className="w-12 h-12 rounded-xl bg-white border border-slate-200/80 flex items-center justify-center mb-3 shadow-2xs group-hover:border-sky-300 transition">
                  {getIcon(skill.name)}
                </div>
                <h3 className="text-sm font-bold text-slate-900 mb-0.5">{skill.name}</h3>
                <span className="text-[11px] font-medium text-slate-500">{skill.type}</span>
              </motion.div>
            ))}
          </AnimatePresence>
        </motion.div>
