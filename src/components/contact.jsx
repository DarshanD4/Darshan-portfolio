...
function Contact() {
  ...
  const contactChannels = [
    ...
    },
    {
      ...
      badge: "Code & Projects",
      icon: (
        <svg className="w-6 h-6 text-slate-800" fill="none" stroke="currentColor" strokeWidth="2.2" viewBox="0 0 24 24">
  ...
  return (
    <section id="contact" className="py-24 px-4 sm:px-8 bg-slate-50/70 border-t border-slate-200/70">
      <div className="max-w-6xl mx-auto">
          ...
          {contactChannels.map((channel) => {
            if (channel.isCopy) {
              return (
                <div
                  ...
                  className="pro-card p-6 bg-white flex flex-col justify-between cursor-pointer group hover:border-sky-300 relative"
                >
                  <div>
                    <div className="flex items-center justify-between gap-2 mb-4">
                      <div className="w-11 h-11 rounded-xl bg-slate-50 border border-slate-200/80 flex items-center justify-center shadow-2xs group-hover:border-sky-300 transition">
                        {channel.icon}
                      </div>
                    ...
                    <h3 className="text-base font-bold text-slate-900 mb-1">{channel.name}</h3>
                    <p className="text-xs font-mono-code text-slate-600 break-all mb-6">
                      {channel.value}
                    </p>
                  </div>
                  ...
                  <AnimatePresence>
                    {copied && (
                      <motion.div
                        ...
                        className="absolute -top-12 left-1/2 -translate-x-1/2 bg-slate-900 text-white text-xs font-bold px-3 py-1.5 rounded-lg shadow-lg whitespace-nowrap z-20"
                      >
            ...
            return (
              <a
                ...
                target="_blank"
                rel="noreferrer"
                className="pro-card p-6 bg-white flex flex-col justify-between group hover:border-sky-300 transition"
              >
                <div>
                  <div className="flex items-center justify-between gap-2 mb-4">
                    <div className="w-11 h-11 rounded-xl bg-slate-50 border border-slate-200/80 flex items-center justify-center shadow-2xs group-hover:border-sky-300 transition">
        ...
        {/* Availability Banner */}
        <div className="mt-12 max-w-3xl mx-auto p-6 rounded-2xl bg-white border border-slate-200/80 shadow-xs flex flex-col sm:flex-row items-center justify-between gap-4 text-center sm:text-left">
          <div className="flex items-center gap-3">
            <div className="w-3 h-3 rounded-full bg-emerald-500 animate-pulse shrink-0" />
            <p className="text-sm font-medium text-slate-700">
              <strong className="text-slate-900">Current Status:</strong> Open for mobile development & AI engineering collaborations.
            </p>
          </div>
